(ns retailops.operation
  "OperationActor -- one retail operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (RetailOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Retail Governor (:govern) and the rollout phase
  ...
  One graph run = one retail operation (intake -> advise -> govern ->
  ...
  decision to a human operator (shop owner/authorized clerk). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:sale/post`/`:reorder/commit` ALWAYS reach this node
  when the governor is clean -- see `retailops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [retailops.retailopsllm :as retailopsllm]
            [retailops.governor :as governor]
...
(defn build
  ...
  `retailops.store/Store`).
  opts:
    :advisor      -- a `retailops.retailopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (retailopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RetailOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (retailopsllm/-advise advisor store request)]
            {:proposal p :audit [(retailopsllm/trace request p)]})))
