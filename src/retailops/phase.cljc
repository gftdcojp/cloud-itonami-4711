(ns retailops.phase
  ...
  (`:order/intake`) -- no separate no-capital-risk 'file' lifecycle
  distinct from the order itself.")

(def read-ops  #{})
(def write-ops #{:order/intake :jurisdiction/assess :sale/post :reorder/commit})
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                       :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:order/intake :jurisdiction/assess}                       :auto #{}}
